...
def sqlConnect():
    ...
    while True:
        decision = input(
            "choose what you would like to do (url, download, remove): "
        )
        ...
        if parameter == "altitude":
            ...
            if decision == "remove":
                sql = (
                    "SELECT * FROM image WHERE altitude >= '%s' and altitude <= '%s'"
                    % (minAltitude, maxAltitude)
                )
                cnxcursor.execute(sql)
                results = cnxcursor.fetchall()
                print(
                    "Here are are the results where: '%s' <= altitude <= '%s'"
                    % (minAltitude, maxAltitude)
                )
                ...
                if len(results) > 0:
                    ...
                    if ask == "y":
                        sql = (
                            "DELETE FROM image WHERE altitude >= '%s' and altitude <= '%s'"
                            % (minAltitude, maxAltitude)
                        )
                        cnxcursor.execute(sql)
                        results = cnxcursor.fetchall()
            ...
            if decision == "remove":
                sql = (
                    "SELECT * FROM image WHERE date >= '%s' and date <= '%s'"
                    % (startDate_form, endDate_form)
                )
                cnxcursor.execute(sql)
                results = cnxcursor.fetchall()
                print(
                    "Here are are the results where '%s' <= date <= '%s'"
                    % (startDate_form, endDate_form)
                )
                printResults(results)
                if len(results) > 0:
                    ...
                    if ask == "y":
                        sql = (
                            "DELETE FROM image WHERE date >= '%s' and date <= '%s'"
                            % (startDate_form, endDate_form)
                        )
                        cnxcursor.execute(sql)
                        results = cnxcursor.fetchall()
            ...
            if decision == "remove":
                ...
                results = cnxcursor.fetchall()
                print(
                    "Here are are the results where ",
                    parameter,
                    " = ",
                    selection,
                )
